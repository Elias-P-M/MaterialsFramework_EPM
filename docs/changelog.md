...
- Added `RandomCalculator` — a dependency-free testing stub returning random energies and forces
- Added `BondLatticeParameter` tool — bond-length-based lattice parameter prediction for FCC, BCC, and HCP alloys (Tandoc et al., Materialia 2025)
- Added entry-point-backed registries for all four domains (`get_calculator`, `get_analyzer`, `get_transformation`, `get_tool`)
